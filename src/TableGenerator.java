...
public class TableGenerator {
	...
	public final static int OPT_BATCH_SIZE = 3;
...
//        	Date now = Calendar.getInstance().getTime();  
        	...
        	int sequenceTableIndex = 1, prevTableIndex = 1, dayCount = 0;
        	...
        	while (true){
        		...
        		if (deliveryCalendar.getTime().getDay() <= 5 && deliveryCalendar.getTime().getDay() >= 1) {
        			...
        			/*
        			 * Write the preview quantities to table:
        			 */
//        			for (int i=0; i<numProds; i++){
//        				if (deliveryCalendar.getTime().getDay() <= 5 && deliveryCalendar.getTime().getDay() >= 1){
//        					
//        				} else {
//        					//Skip weekends, i.e. fill rows with quantities = 0:
//        					Number TypeQuantity = new Number(i+1, prevTableIndex, 0); 
//        					previewTable.addCell(TypeQuantity);
//        				}
//                	}
        			...
        			/*
        			 * Calculate the quantities per calloff based on the preview quantities:
        			 */
//        			int[] calloffQuantities = new int[previewQuantities.length];
//        			int sumCalloffQuantities = 0;
//        			for (int i = 0; i < previewQuantities.length; i++){
//       				 	sumCalloffQuantities = sumCalloffQuantities + previewQuantities[i];
//        			}
//        			int sumBaseQuantities = 0;
//        			for (int i = 0; i < previewQuantities.length; i++){
//        				 int divQty = previewQuantities[i];
//        				 calloffQuantities[i] = divQty;
//        				 sumBaseQuantities += divQty;
//        			}
//        			int remQty = sumCalloffQuantities - sumBaseQuantities;
//        			int i = 0;
//        			while (remQty > 0){
//        				calloffQuantities[i]++;
//        				remQty--;
...
//        			}
        			 ...
        			 */
//        			int[] remainingQuantities = arrayCopyWithDeviation(calloffQuantities, deviation);
...
//		for (int j=0; !isZeroArray(remainingQuantities); j++){
	...
	public static int[] linearDemandCurve(int[] baseQuantities, int[] growth, int currentDay, int maxDayCount){
    	...
    	for (int i = 0; i < baseQuantities.length; i++){
    		result[i] = (int) (baseQuantities[i] + currentDay * (growth[i] * baseQuantities[i] ) / (100 * maxDayCount) );
    	}
    	return result;
    }
...
//    	return Arrays.asList(array).contains(elem);
    ...
    public static int[] arrayCopyWithDeviation(int[] array, int maxDeviation){
    	int[] newArray = new int[array.length];
    	Random random = new Random();
    	for (int i = 0; i < array.length; i++){
    		int deviation = getRandomWithExclusion(random, -maxDeviation, maxDeviation, new int[]{});
...
//    		System.out.println("abs max dev = " + absoluteMaxDev);
//    		int deviation = getRandomWithExclusion(random, -maxDev[i], maxDev[i], new int[]{});
    		int deviation = getRandomWithExclusion(random, -absoluteMaxDev, absoluteMaxDev, new int[]{});
    		
//    		int devRange = absoluteMaxDev * 2;
//    		int deviation = random.nextInt(devRange) - absoluteMaxDev;
    ...
    public static int[] arrayAppendFirst(int[] array, int newElement){
    	int[] newArray = new int[array.length + 1];
...
//    		System.out.println(newArray[i]);
    ...
    private static void arrayPrint(String name, int[] array){
    	String out = "";
